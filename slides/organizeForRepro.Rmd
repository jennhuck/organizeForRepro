...
::: {.notes}
This assumes that you are using scripts.  You definitely should be using scripts for any changes or analysis that you make on your project.  Using GUIs to point and click or using Excel is not good for logging what you did (which affects transparency of your project.) 
:::
...
::: {.notes}
The original data files are files that you received or created, and did not modify in any way.  This represents the very start of your project.  Everything about the project can be traced back to this original dataset. 

Treat raw data as read-only - never overwrite your data.  
Better: Using your operating system explorer, mark your raw data read-only, so you cannot inadvertently change the file.  
Best: put a backup copy somewhere else. UVA makes OneDrive and Box  available to the academic side.

*Show how to make read-only in Windows Explorer.*

A modified data file is one that you change somehow.  These changes might include some sort of data cleaning.  The modified data file might look significantly different from the raw ones.  It is preferable to use scripts to make your modifications.  Scripts are not only more efficient than manual editing, they also are a log of the changes you made to the original data file.

Treat modified data as disposable.  The scripts allow you to completely recreate your modified data.
...
::: {.notes}
Treat generated output as disposable.  Just like modified data, you can rely on your scripts to re-create any figures or graphs. 
:::
...
- Original files (raw data, scripts) and output files (modified data, tables and plots) should be clearly distinguished.
  - Keep raw files raw - never edit original copies.
  - Treat output files as dispensable.
...
::: {.notes}
The processed data, figures, and stats should all be easily regenerated, as long as you have the original data and scripts.  Treat them as disposable.  (I’m less sure the paper could be easily regenerated...)
:::
...
## Project Organization
- **Good**. Separate input and code from output using different folders or file names and keep track of all the input->output changes in separate files. 
...
Inside that make three more folders:
...
Click on the big green CODE button, download the zipped version, and extract it.  You could set up something like thi for your own project
:::
...
::: {.notes}
Every project should describe to users what the purpose of the project is. This is commonly done in a README file. As the starting point for a project the README file is formatted as plain text (or [markdown](https://guides.github.com/features/mastering-markdown/)) ([markdown cheat sheet](https://guides.github.com/pdfs/markdown-cheatsheet-online.pdf)) to make it easily readable. 
...
Text editor recommendations: [Notepad++](http://notepad-plus-plus.org/) or [BBEdit/TextWrangler](http://www.barebones.com/products/textwrangler/download.html), or [Sublime Text](https://www.sublimetext.com). 
...
- Do use:
  ...
  - Seperate units of metadata with `_` -- use it as a delimiter for regex
...
- **Best**. Seperate units of metadata with `_` (underscrore) for regex. Within units of metadata, use `-` (hyphen or dash) for human readability. 
...
We didn’t talk much about version control, but this project can definitely benefit from that.  Keep an eye out for UVA Library’s GitHub workshops. (There is one offered next Tuesday!) 
:::
